...
```scan gamepath/common -> scan gamepath/history -> scan gamepath/events -> scan gamepath/interface -> scan gamepath/gfx```.  
...
Program will scan content of files, if they extensions are `*.txt`, `*.gui`,`*.gfx` or `*.yml`. Also, program will check every filename, and add they to list too, if filename contains entired searchword. Results of matching filename will displayed with mark (see screenshot)
